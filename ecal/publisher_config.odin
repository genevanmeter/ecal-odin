...
* On publisher side the memory copy is executed into the opened memory file. On the subscriber side the user message
...
Publisher_Layer_SHM_Configuration :: struct {
	enable:                  c.int,  //!< enable layer 
	zero_copy_mode:          c.int,  //!< Enable zero copy shared memory transport mode 
	acknowledge_timeout_ms:  c.uint, /*!< Force connected subscribers to send acknowledge event after processing the message.
                                            The publisher send call is blocked on this event with this timeout (0 == no handshake).*/
	memfile_buffer_count:    c.uint, /*!< Maximum number of used buffers (needs to be greater than 1, default = 1) */
	memfile_min_size_bytes:  c.uint, //!< Default memory file size for new publisher (Default: 4096)
	memfile_reserve_percent: c.uint, //!< Dynamic file size reserve before recreating memory file if topic size changes (Default: 50)
}

Publisher_Layer_UDP_Configuration :: struct {
	enable: c.int, //!< enable layer
}

Publisher_Layer_TCP_Configuration :: struct {
	enable: c.int, //!< enable layer
}

Publisher_Layer_Configuration :: struct {
	shm: Publisher_Layer_SHM_Configuration,
	udp: Publisher_Layer_UDP_Configuration,
	tcp: Publisher_Layer_TCP_Configuration,
}

Publisher_Configuration :: struct {
	layer:                        Publisher_Layer_Configuration, //!< Layer configuration
	layer_priority_local:         [8]TransportLayer_eType,
	layer_priority_local_length:  c.size_t,
	layer_priority_remote:        [8]TransportLayer_eType,
	layer_priority_remote_length: c.size_t,
}
